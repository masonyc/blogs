...
![Figure 5-10 Quorum write, quorum read, and read repair after node outage](../assets/ddia/Figure-5.10.jpg)
...
![Figure 5-11 If w + r > n, at least one of the r replicas you read from must have seen the most recent successful write.](../assets/ddia/Figure-5.11.jpg)
...
- If two writes occur concurrently, it is not clear which one happened first. In this case, the only safe solution is to merge the concurrent writes (see “Handling Write Conflicts”). If a winner is picked based on a timestamp (last write wins), writes can be lost due to clock skew.
...
- Is it better to return errors to all requests for which we cannot reach a quorum of w or r
  nodes?
...
> Think n is the quorum for a partitioned value, not necessary every node in a cluster.
